## v1.14.5 (2026-03-09)
- **机器码绑定授权**：许可证升级为机器码绑定模式，新增机器码显示、激活页、About 授权状态与剩余天数展示。
- **双版本发布**：持续提供 `license` / `nolicense` 双版本，并保留独立 `KeyGen` 工具。
- **结果视图优化**：单用户扫描时隐藏 `SMTPAddress` / `UPN`；多邮箱扫描时仅显示 CSV 实际标识列。
- **日志与界面调整**：日志级别控件位置优化，窗口标题可显示注册状态。
- **会议扫描增强**：
  - 会议模式下禁用文件夹选择和搜索详细度。
  - 结果新增 `GOID` / `CleanGOID`。
  - 支持按 `UserRole` 执行 `Cancel` / `Decline`，并自动隐藏不适用操作。
- **筛选条件增强**：
  - Meeting：支持 `GOID`、`CleanGOID`、`与会者包含地址`。
  - Email：支持 `是否包含附件`、`收件人地址包含`。
